"""Verify ASS title generation handles chapter + location + person (no timeline)."""
import sys
...
events = [
    ...
    {"kind": "person", "start": 8.5, "text": "STEFAN MANDEL", "para_idx": 1},  # collides w/ location
]
...
# style checks
for style in ("TypePerson", "TypePersonGhost", "TypeLoc", "ChapCore"):
    assert style in txt, f"missing style {style}"
assert "TypeTime" not in txt, "TypeTime style should be gone"
print("styles OK: TypePerson gold + TypePersonGhost present, TypeTime removed")
...
# stacking: person at 8.5 collides with location (8.2) -> _stack_up=1 -> base_y = H-110-74
import re
